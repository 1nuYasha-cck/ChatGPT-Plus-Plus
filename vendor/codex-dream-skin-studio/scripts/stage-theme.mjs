...
const [sourceDirArg, stageDirArg, destinationImageArg] = process.argv.slice(2);
...
async function main() {
  ...
  const destinationImage = destinationImageArg || theme.image;
  if (path.basename(destinationImage) !== destinationImage || destinationImage === "theme.json") {
    throw new Error("Staged theme image name is invalid");
  }
  assertContained(stageRoot, path.join(stageRoot, destinationImage), "Staged theme image");
  ...
  const stagedTheme = { ...theme, image: destinationImage };
  await writeExclusive(path.join(stageRoot, destinationImage), image.bytes);
  await writeExclusive(path.join(stageRoot, "theme.json"), Buffer.from(`${JSON.stringify(stagedTheme, null, 2)}\n`));
  process.stdout.write(destinationImage);
}
