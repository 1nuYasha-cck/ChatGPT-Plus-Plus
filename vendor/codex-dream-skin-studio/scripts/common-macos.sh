...
ensure_private_directory() {
  ...
  [ ! -L "$target" ] || fail "$label must not be a symbolic link: $target"
  if [ -e "$target" ] && [ ! -d "$target" ]; then
    fail "$label must be a directory: $target"
  fi
  /bin/mkdir -p "$target"
  [ -d "$target" ] && [ ! -L "$target" ] || fail "$label could not be created safely: $target"
  /bin/chmod 700 "$target"
}

ensure_state_root() {
  ensure_private_directory "$STATE_ROOT" "State root"
  local child=""
  for child in "$THEME_DIR" "$STATE_ROOT/themes" "$STATE_ROOT/images"; do
    ensure_private_directory "$child" "Dream Skin state child"
  done
}
...
codex_main_pids() {
  ...
  done < <(LC_ALL=en_US.UTF-8 /bin/ps -axo pid=,command=)
}
...
process_started_at_epoch() {
  local started_at=""
  started_at="$(process_started_at "$1")"
  [ -n "$started_at" ] || return 1
  LC_ALL=C /bin/date -j -f '%a %b %e %T %Y' "$started_at" '+%s' 2>/dev/null
}
...
iso8601_epoch() {
  ...
  value="${value%Z}"
  [ -n "$value" ] || return 1
  TZ=UTC LC_ALL=C /bin/date -j -u -f '%Y-%m-%dT%H:%M:%S' "$value" '+%s' 2>/dev/null
}
...
recorded_injector_process_matches() {
  ...
  local actual_start=""
  local actual_start_epoch=""
  local expected_start_epoch=""
  local state_created_at=""
  local state_created_epoch=""
  local epoch_delta=""
  ...
  # Quota may start with the C locale. macOS `ps` then renders UTF-8 paths as
  # M-eM^... escape text, so a watcher launched from a Chinese directory no
  # longer matches its byte-identical recorded path. Force a UTF-8 locale for
  # command text; lstart remains separately normalized with LC_ALL=C.
  command_line="$(LC_ALL=en_US.UTF-8 /bin/ps -p "$pid" -o command= 2>/dev/null || true)"
  ...
  actual_start_epoch="$(process_started_at_epoch "$pid" 2>/dev/null || true)"
  expected_start_epoch="$(state_field injectorStartedAtEpoch 2>/dev/null || true)"
  case "$expected_start_epoch" in
    ''|*[!0-9]*) ;;
    *) [ -n "$actual_start_epoch" ] && [ "$actual_start_epoch" = "$expected_start_epoch" ] && return 0 ;;
  esac
  [ "$actual_start" = "$expected_start" ] && return 0

  # v1.2.0 stored `ps lstart` using the caller locale. A state written under
  # zh_CN therefore cannot equal the same process time later rendered under
  # the C locale. For that legacy schema only, bind the already-validated
  # command/PID tuple to state.createdAt; write_state records it within a few
  # seconds of spawning the watcher. This migrates the locale mismatch without
  # weakening the reused-PID guard for incomplete or hand-written records.
  state_created_at="$(state_field createdAt 2>/dev/null || true)"
  state_created_epoch="$(iso8601_epoch "$state_created_at" 2>/dev/null || true)"
  case "$actual_start_epoch:$state_created_epoch" in
    *[!0-9:]*|:|*:|:*) return 1 ;;
  esac
  epoch_delta=$((state_created_epoch - actual_start_epoch))
  [ "$epoch_delta" -ge 0 ] && [ "$epoch_delta" -le 5 ]
}
...
pid_is_codex_descendant() {
  ...
  while [ "$current" -gt 1 ] 2>/dev/null && [ "$depth" -lt 32 ]; do
    command_line="$(LC_ALL=en_US.UTF-8 /bin/ps -p "$current" -o command= 2>/dev/null || true)"
    case "$command_line" in
...
write_state() {
  ...
  local injector_started_at_epoch=""
  local node_ver="${NODE_VERSION:-unknown}"
  local bundle="${CODEX_BUNDLE:-}"
  local exe="${CODEX_EXE:-}"
  local app_ver="${CODEX_VERSION:-}"
  local team="${CODEX_TEAM_ID:-}"
  injector_started_at_epoch="$(process_started_at_epoch "$injector_pid")" \
    || fail "Could not record the injector process start epoch."
  "$NODE" -e '
    const fs = require("node:fs");
    const [file, version, port, pid, startedAt, startedAtEpoch, injector, node, nodeVersion, bundle, exe, appVersion, teamId, root, themeDir, codexPid, arch, session] = process.argv.slice(1);
    const state = {
      schemaVersion: 4,
      ...
      injectorStartedAtEpoch: Number(startedAtEpoch),
  ...
  ' "$STATE_PATH" "$SKIN_VERSION" "$port" "$injector_pid" "$injector_started_at" "$injector_started_at_epoch" "$INJECTOR" "$NODE" "$node_ver" "$bundle" "$exe" "$app_ver" "$team" "$PROJECT_ROOT" "$THEME_DIR" "$codex_pid" "$(/usr/bin/uname -m)" "$session"
}
...
validated_recorded_injector_pid() {
  local expected_port="${1:-}"
  local pid saved_port saved_start saved_node saved_injector saved_version expected_node_real loaded_node_real
  [ -f "$STATE_PATH" ] || return 1
  ...
  case "$pid:$saved_port" in *[!0-9:]*|:|*:|:*) return 1 ;; esac
  [ "$pid" != "0" ] && [ -n "$saved_start" ] && [ -n "$saved_node" ] \
    && [ -n "$saved_injector" ] || return 1
  [ -z "$expected_port" ] || [ "$saved_port" = "$expected_port" ] || return 1
  [ "$saved_version" = "$SKIN_VERSION" ] || return 1
  # Never reuse a watcher whose Node path belongs to an older ChatGPT build.
  # The command line can retain the stable /Applications path while the loaded
  # executable still points into an obsolete Sparkle update cache.
  [ -n "${NODE:-}" ] && [ "$saved_node" = "$NODE" ] || return 1
  /bin/kill -0 "$pid" 2>/dev/null || return 1
  recorded_injector_process_matches \
    "$pid" "$saved_start" "$saved_node" "$saved_injector" "$saved_port" || return 1
  expected_node_real="$(cd "$(/usr/bin/dirname "$NODE")" && pwd -P)/$(/usr/bin/basename "$NODE")"
  loaded_node_real="$(/usr/sbin/lsof -a -p "$pid" -d txt -Fn 2>/dev/null \
    | /usr/bin/sed -n 's/^n//p' | /usr/bin/head -n 1)"
  [ -n "$loaded_node_real" ] && [ "$loaded_node_real" = "$expected_node_real" ] || return 1
  /usr/bin/printf '%s\n' "$pid"
}
...
stop_recorded_injector() {
  ...
  if ! recorded_injector_process_matches "$pid" "$saved_start" "$saved_node" "$saved_injector" "$saved_port"; then
    ...
    if ! /bin/kill -0 "$pid" 2>/dev/null || [ -z "$(LC_ALL=en_US.UTF-8 /bin/ps -p "$pid" -o command= 2>/dev/null || true)" ]; then
      /bin/launchctl remove "$INJECTOR_JOB_LABEL" >/dev/null 2>&1 || true
      return 0
    fi
...
launch_injector_daemon() {
  ...
  : > "$INJECTOR_ERROR_LOG"
  /bin/launchctl remove "$INJECTOR_JOB_LABEL" >/dev/null 2>&1 || true
  # The watcher is intentionally not a KeepAlive launchd job. It exits after
  # a bounded CDP outage so an app update or normal quit cannot leave a
  # respawning process that repeatedly probes a dead port.
...
hot_reapply_theme() {
  ...
  if [ "$injector_protocol" = "2" ] || [ "$injector_protocol" = "3" ]; then
    inj_pid="$(LC_ALL=en_US.UTF-8 /bin/ps -axo pid=,command= | /usr/bin/awk -v inj="$INJECTOR" -v port="$port" '
  ...
  if [ -n "$inj_pid" ] && /bin/kill -0 "$inj_pid" 2>/dev/null \
    ...
    # The watcher can miss a success state written during its startup/reload
    # window. Notify the active renderer directly so its loading overlay is
    # always moved to a terminal state and then removed.
    finish_client_operation "$port" success "皮肤已应用" "$operation_token" 1500 >/dev/null 2>&1 || true
    return 0
  fi
  ...
  write_operation_state success "皮肤已应用" "$operation_token" || return 1
  # A freshly started watcher has not necessarily observed the terminal
  # operation state yet; complete the in-renderer status explicitly.
  finish_client_operation "$port" success "皮肤已应用" "$operation_token" 1500 >/dev/null 2>&1 || true
  return 0
}
...
launch_codex_with_cdp() {
  ...
  /usr/bin/open -n "$CODEX_BUNDLE" --args \
    --remote-debugging-address=127.0.0.1 \
    --remote-debugging-port="$port" \
    >>"$APP_LOG" 2>>"$APP_ERROR_LOG" || true
  # Give LaunchServices a bounded chance to create the main process before
  # deciding whether the direct executable fallback is necessary.
  deadline=$((SECONDS + 3))
  while ! codex_is_running && [ "$SECONDS" -lt "$deadline" ]; do /bin/sleep 0.15; done
  if ! codex_is_running; then
