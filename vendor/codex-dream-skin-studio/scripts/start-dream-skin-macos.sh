...
record_start_exit() {
  ...
  local failure_message="应用失败，应用结果未确认"
  ...
  # If this transaction closed ChatGPT but failed before the themed launch
  # became usable, restore the application in normal mode. Keep the original
  # failure exit code and diagnostics: recovery is a safety net, not success.
  if [ "${CODEX_STOPPED_BY_START:-false}" = "true" ] && ! codex_is_running; then
    if launch_codex_normally >/dev/null 2>&1; then
      recovery_deadline=$((SECONDS + 10))
      while ! codex_is_running && [ "$SECONDS" -lt "$recovery_deadline" ]; do
        /bin/sleep 0.25
      done
      if codex_is_running; then
        failure_message="应用失败，ChatGPT 已恢复启动"
      else
        failure_message="应用失败，ChatGPT 恢复启动未确认"
      fi
    else
      failure_message="应用失败，ChatGPT 恢复启动失败"
    fi
  fi
  ...
  write_operation_state failed "$failure_message" "${OPERATION_TOKEN:-}" 2>/dev/null || true
  finish_client_operation "${PORT:-9341}" error "$failure_message" \
...
INJECTOR_PID=""
# Keep a healthy watcher on a live CDP renderer. Replacing it on every theme
# switch created operation-UI races and made a completed apply look stuck.
# A different engine version is never reused, so app updates still replace the
# old process deterministically.
if [ -f "$STATE_PATH" ]; then
  if [ "$DEBUG_READY" = "true" ]; then
    INJECTOR_PID="$(validated_recorded_injector_pid "$PORT" 2>/dev/null || true)"
  fi
  if [ -z "$INJECTOR_PID" ]; then
    # Validate and stop before closing ChatGPT. A stale or identity-mismatched
    # PID must fail while the application is still open.
    stop_recorded_injector
  fi
fi

if [ "$NEEDS_CODEX_RESTART" = "true" ]; then
  stop_codex true
...
if [ "$DEBUG_READY" = "false" ]; then
  ...
  /usr/bin/open "$CODEX_BUNDLE" >/dev/null 2>&1 || true
