...
injector_identity_matches() {
  ...
  local expected_injector="$6"
  local expected_port="$7"
  local command_line command_lower node_lower injector_lower actual_start actual_epoch created_value created_epoch epoch_delta
  ...
  actual_epoch="$(LC_ALL=C /bin/date -j -f '%a %b %e %T %Y' "$actual_start" '+%s' 2>/dev/null || true)"
  case "$expected_start_epoch" in
    ''|*[!0-9]*) ;;
    *) [ -n "$actual_epoch" ] && [ "$actual_epoch" = "$expected_start_epoch" ] && return 0 ;;
  esac
  [ "$actual_start" = "$expected_start" ] && return 0
  created_value="${expected_created_at%%.*}"
  created_value="${created_value%Z}"
  created_epoch="$(TZ=UTC LC_ALL=C /bin/date -j -u -f '%Y-%m-%dT%H:%M:%S' "$created_value" '+%s' 2>/dev/null || true)"
  case "$actual_epoch:$created_epoch" in
    *[!0-9:]*|:|*:|:*) return 1 ;;
  esac
  epoch_delta=$((created_epoch - actual_epoch))
  [ "$epoch_delta" -ge 0 ] && [ "$epoch_delta" -le 5 ]
}

# `pgrep -x` is unreliable for the current Chromium launcher because macOS can
# expose a truncated process name. Match the official main executable command
# instead; helper processes live below Contents/Frameworks and do not qualify.
...
done < <(LC_ALL=en_US.UTF-8 /bin/ps -axo command= 2>/dev/null)
...
if [ -f "$STATE_PATH" ]; then
  ...
  saved_start_epoch="$(read_json_text_field "$STATE_SNAPSHOT" injectorStartedAtEpoch)"
  saved_created_at="$(read_json_text_field "$STATE_SNAPSHOT" createdAt)"
  ...
  if injector_identity_matches "${pid:-}" "$saved_start" "$saved_start_epoch" "$saved_created_at" "$saved_node" "$saved_injector" "$PORT"; then
    INJECTOR_ALIVE="true"
    case "${SESSION:-}" in
