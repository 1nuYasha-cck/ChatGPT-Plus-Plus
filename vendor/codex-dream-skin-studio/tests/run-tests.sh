...
# New image themes always produce a 2560x1440 JPEG without stretching. Wide
# images cover-crop; square/portrait images retain the subject over blur.
IMAGE_CASES="$TMP/image-cases"
/bin/mkdir -p "$IMAGE_CASES"
/bin/cp "$ROOT/presets/preset-arina-hashimoto/background.jpg" "$IMAGE_CASES/landscape.jpg"
/usr/bin/sips -c 1400 700 "$IMAGE_CASES/landscape.jpg" \
  --out "$IMAGE_CASES/portrait.jpg" >/dev/null
/usr/bin/sips -c 1200 1200 "$IMAGE_CASES/landscape.jpg" \
  --out "$IMAGE_CASES/square.jpg" >/dev/null
/usr/bin/sips -z 360 640 "$IMAGE_CASES/landscape.jpg" \
  --out "$IMAGE_CASES/low.jpg" >/dev/null
for image_case in landscape portrait square low; do
  IMAGE_RESULT="$(/usr/bin/osascript -l JavaScript \
    "$ROOT/scripts/prepare-theme-image.jxa" \
    "$IMAGE_CASES/$image_case.jpg" "$IMAGE_CASES/$image_case-output.jpg")"
  IMAGE_WIDTH="$(/usr/bin/sips -g pixelWidth "$IMAGE_CASES/$image_case-output.jpg" \
    | /usr/bin/awk '/pixelWidth:/{print $2}')"
  IMAGE_HEIGHT="$(/usr/bin/sips -g pixelHeight "$IMAGE_CASES/$image_case-output.jpg" \
    | /usr/bin/awk '/pixelHeight:/{print $2}')"
  [ "$IMAGE_WIDTH" = "2560" ] && [ "$IMAGE_HEIGHT" = "1440" ] || {
    printf 'Prepared %s image is not 2560x1440.\n' "$image_case" >&2
    exit 1
  }
  case "$image_case:$IMAGE_RESULT" in
    landscape:*'"layout":"cover-crop"'*) ;;
    portrait:*'"layout":"blurred-contain"'*) ;;
    square:*'"layout":"blurred-contain"'*) ;;
    low:*'"lowResolution":true'*) ;;
    *) printf 'Unexpected image preparation result for %s: %s\n' "$image_case" "$IMAGE_RESULT" >&2; exit 1 ;;
  esac
done

IMAGE_LOAD_HOME="$TMP/image-load-home"
/usr/bin/env HOME="$IMAGE_LOAD_HOME" NODE="$NODE" \
  "$ROOT/scripts/load-image-theme-macos.sh" \
  --file "$IMAGE_CASES/portrait.jpg" --name "Portrait QA" --no-apply >/dev/null
...
# Restart safety contract: the previous injector must be validated/stopped
# before ChatGPT is closed, and a post-close failure must relaunch normally.
START_SCRIPT="$ROOT/scripts/start-dream-skin-macos.sh"
START_CLEANUP_LINE="$(/usr/bin/grep -n '^[[:space:]]*stop_recorded_injector$' "$START_SCRIPT" | /usr/bin/head -n 1 | /usr/bin/cut -d: -f1)"
START_STOP_LINE="$(/usr/bin/grep -n '^[[:space:]]*stop_codex true$' "$START_SCRIPT" | /usr/bin/head -n 1 | /usr/bin/cut -d: -f1)"
[ -n "$START_CLEANUP_LINE" ] && [ -n "$START_STOP_LINE" ] && [ "$START_CLEANUP_LINE" -lt "$START_STOP_LINE" ] || {
  printf 'start script can close ChatGPT before validating the recorded injector.\n' >&2
  exit 1
}
/usr/bin/grep -F -q 'CODEX_STOPPED_BY_START="true"' "$START_SCRIPT"
/usr/bin/grep -F -q 'launch_codex_normally >/dev/null 2>&1' "$START_SCRIPT"
...
# A watcher is reusable only when its recorded process identity, port, and
# engine version all match.  This prevents theme switches from racing a
# healthy watcher while still replacing stale watcher builds after upgrades.
/usr/bin/grep -q 'validated_recorded_injector_pid' "$ROOT/scripts/start-dream-skin-macos.sh"
/usr/bin/grep -q '\[ "$saved_version" = "$SKIN_VERSION" \]' "$ROOT/scripts/common-macos.sh"
/usr/bin/grep -q 'recorded_injector_process_matches' "$ROOT/scripts/common-macos.sh"
...
# v1.2.0 persisted locale-sensitive `ps lstart` text. The same watcher must
# remain identifiable when a later caller renders that timestamp in another
# language; state.createdAt provides the bounded migration proof.
LOCALE_FAKE_DIR="$TMP/公司电脑"
/bin/mkdir -p "$LOCALE_FAKE_DIR"
LOCALE_FAKE_INJECTOR="$LOCALE_FAKE_DIR/locale-fake-injector.mjs"
/usr/bin/printf 'setTimeout(() => {}, 30000);\n' > "$LOCALE_FAKE_INJECTOR"
"$NODE" "$LOCALE_FAKE_INJECTOR" --watch --port 19342 --theme-dir "$TMP" &
STATUS_PID="$!"
/bin/sleep 0.08
"$NODE" -e '
  const fs = require("node:fs");
  ...
  fs.writeFileSync(file, `${JSON.stringify({
    ...
    port: 19342,
    injectorPid: Number(pid),
    injectorStartedAt: "三 7月/22 19:01:59 2026",
    injectorPath: injector,
    nodePath: node,
    createdAt: new Date().toISOString(),
  })}\n`);
' "$STATUS_STATE_ROOT/state.json" "$STATUS_PID" "$NODE" "$LOCALE_FAKE_INJECTOR"
STATUS_JSON="$(/usr/bin/env HOME="$STATUS_HOME" LC_ALL=C "$ROOT/scripts/status-dream-skin-macos.sh" --json)"
"$NODE" -e '
  const value = JSON.parse(process.argv[1]);
  if (value.session !== "active" || value.injectorAlive !== true) process.exit(1);
' "$STATUS_JSON"
/usr/bin/env HOME="$STATUS_HOME" NODE="$NODE" LC_ALL=C /bin/bash -c '
  . "$1/scripts/common-macos.sh"
  pid="$(state_field injectorPid)"
  if ! recorded_injector_process_matches "$pid" "$(state_field injectorStartedAt)" \
    "$(state_field nodePath)" "$(state_field injectorPath)" "$(state_field port)"; then
    exit 1
  fi
' _ "$ROOT"
/bin/kill -TERM "$STATUS_PID" 2>/dev/null || true
