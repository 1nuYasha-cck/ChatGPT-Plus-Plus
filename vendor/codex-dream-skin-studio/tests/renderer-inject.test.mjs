...
assert.doesNotMatch(
  css,
  /main\.dream-skin-main-surface\s*>\s*header\.app-header-tint\s*\{[^}]*\b(?:position|z-index)\s*:/,
  "The skin must preserve Codex's native fixed header so the side-panel toggle remains reachable.",
);
assert.doesNotMatch(
  css,
  /main\.dream-skin-main-surface:not\(\.dream-skin-home-shell\)\s*>\s*\*\s*\{[^}]*\bposition\s*:/,
  "Task-route child layering must not overwrite the native header position.",
);
assert.match(
  css,
  /\.dream-skin-home\s*>\s*div:first-child\s*\{[^}]*min-height:\s*0 !important;[^}]*padding-top:\s*0 !important;/,
  "The Codex 26.721 empty home-banners wrapper must retain zero height.",
);
assert.doesNotMatch(
  css,
  /\.dream-skin-home\s*>\s*div:first-child\s*\{[^}]*min-height:\s*100% !important;/,
  "The first home child must not consume a full viewport before the real home content.",
);
...
assert.match(
  css,
  /data-dream-art-wide="true"\]:has\(main\.dream-skin-main-surface\.dream-skin-home-shell\)[\s\S]{0,100}body\s*\{[\s\S]{0,300}background-image:\s*var\(--dream-skin-art\) !important;/,
  "Wide home artwork should use the same full-window image as utility routes.",
);
assert.match(
  css,
  /data-dream-art-wide="true"\]:has\(main\.dream-skin-main-surface\.dream-skin-home-shell\)[\s\S]{0,120}body\s*\{[\s\S]{0,260}background-position:\s*var\(--ds-art-position\) !important;/,
  "Wide home artwork must honor the configured focal point instead of forcing a centered crop.",
);
assert.match(
  css,
  /data-dream-art-task-mode="ambient"[\s\S]{0,260}data-dream-art-wide="true"\]:has\(main\.dream-skin-main-surface:not\(\.dream-skin-home-shell\)\)[\s\S]{0,120}body\s*\{[\s\S]{0,260}background-position:\s*var\(--ds-art-position\) !important;/,
...
function createFixture(theme, {
  ...
  const document = {
    ...
    querySelector(selector) {
      if (selector === "main.dream-skin-main-surface" || selector === "main.main-surface" || selector === "main") return shellMain;
      return null;
    },
    querySelectorAll(selector) {
      if (selector === "main.dream-skin-main-surface" && shellMain.classList.contains("dream-skin-main-surface")) {
        return [shellMain];
      }
      return [];
    },
