((cssText, artDataUrl, themeConfig) => {
  ...
  const MAIN_CLASS = "dream-skin-main-surface";
  ...
  const findShellMain = () => document.querySelector(`main.${MAIN_CLASS}`) ||
  ...
  const syncRouteState = (shell, { layout = false } = {}) => {
    ...
    const heroSource = home?.querySelector('[data-feature="game-source"]') || null;
    const suggestions = home?.querySelector('.group\\/home-suggestions') || null;
    for (const node of document.querySelectorAll(".dream-skin-home-hero, .dream-skin-home-suggestions")) {
      node.classList.remove("dream-skin-home-hero", "dream-skin-home-suggestions");
    }
    if (heroSource) {
      // Select the smallest meaningful container instead of relying on
      // first-child chains; Codex now inserts optional banner wrappers.
      let hero = heroSource.parentElement;
      while (hero?.parentElement && hero.parentElement !== home && hero.children.length === 1) hero = hero.parentElement;
      hero?.classList.add("dream-skin-home-hero");
    }
    suggestions?.classList.add("dream-skin-home-suggestions");
    ...
    for (const candidate of document.querySelectorAll(`main.${MAIN_CLASS}`)) {
      if (candidate !== shellMain) candidate.classList.remove(MAIN_CLASS);
    }
    shellMain.classList.add(MAIN_CLASS);
  ...
  const cleanup = () => {
    ...
    document.querySelectorAll(`main.${MAIN_CLASS}`).forEach((node) => node.classList.remove(MAIN_CLASS));
    document.querySelectorAll(".dream-skin-home-utility").forEach((node) => node.classList.remove("dream-skin-home-utility"));
    document.querySelectorAll(".dream-skin-home-hero, .dream-skin-home-suggestions").forEach((node) =>
      node.classList.remove("dream-skin-home-hero", "dream-skin-home-suggestions"));
